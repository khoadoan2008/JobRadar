...
### 📝 Ngày 28/06/2026 - Chủ đề: Khắc phục lỗi HTTP 400 Bad Request (Tomcat HTML thô) trong giao tiếp giữa các Microservice

*   **Khái niệm mới học được (Hiểu theo ý mình):**
    *   **OpenFeign Client** là một thư viện của Spring Cloud giúp gọi API nội bộ giữa các microservice bằng Interface rất trực quan. Tuy nhiên, nó tự động serialize/deserialize và tự gán headers theo cấu hình ngầm, đôi khi gây ra các header xung đột hoặc dư thừa.
    *   **Tomcat 10.1 (Spring Boot 3)** áp dụng chuẩn bảo mật RFC 7230 cực kỳ nghiêm ngặt. Nếu request body chứa các **ký tự điều khiển ASCII đặc biệt ẩn (Control Characters từ `0x00` đến `0x1F`)** (do nhà tuyển dụng copy từ Word/PDF vào tin tuyển dụng) hoặc URL chứa ký tự tiếng Việt có dấu chưa được encode, Tomcat sẽ từ chối nhận request ngay từ tầng mạng. Nó trả về lỗi 400 HTML thô thay vì trả về định dạng JSON lỗi thông thường của Spring Boot.
    *   **RestTemplate** là công cụ HTTP client truyền thống của Spring. Nó cung cấp cơ chế kiểm soát trực tiếp và ổn định hơn đối với dữ liệu/header được gửi đi, là sự thay thế đáng tin cậy khi Feign Client gặp lỗi cấu hình ngầm.

*   **Đoạn code / Cấu hình tâm đắc:**
    *   *Hàm Regex dọn dẹp triệt để các ký tự điều khiển ASCII đặc biệt ẩn (chỉ giữ lại xuống dòng, tab):*
    ```java
    ...
    ```
    *   *Dựng URL ASCII sạch bằng slug (alias) và jobId thay vì lấy URL gốc có thể dính tiếng Việt:*
    ```java
    String jobUrl = "https://www.vietnamworks.com/" + alias + "-" + jobId + "-jv";
    ```
    *   *Gửi POST request lưu tin bằng RestTemplate:*
    ```java
...
*   **Bug / Khó khăn gặp phải & Cách khắc phục:**
    *   *Mô tả lỗi:* Khi chạy cào VietnamWorks, log của `crawler-service` bắn ra lỗi: `[400] during [POST] to [http://localhost:8083/api/v1/jobs] [JobClient#createJob(JobRequest)]: [<!doctype html><html lang="en"><head><title>HTTP Status 400 – Bad Request</title>... (435 bytes)]`. Tất cả các tin cào được đều thất bại.
    *   *Cách giải quyết:* 
        1. **Về Dữ liệu**: Ký tự điều khiển rác ẩn trong mô tả công việc và khoảng trắng hoặc ký tự unicode tiếng Việt có dấu trong URL làm hỏng parser của Tomcat. Đã sửa đổi bằng cách dọn dẹp text bằng Regex và tự dựng URL sạch 100% ASCII từ `alias` + `jobId`.
        2. **Về Giao tiếp**: Cấu hình header tự sinh của Feign Client bị xung đột ngầm với Tomcat 10 của `job-service`. Đã khắc phục triệt để bằng cách chuyển đổi cả `ViecLamTotCrawler` và `VietnamWorksCrawler` sang dùng `RestTemplate` để gửi POST request sang `job-service`, giúp hệ thống chạy ổn định và kiểm soát tốt hơn.

---

### 📝 Ngày 28/06/2026 - Chủ đề: Sửa lỗi Parse Error JSX (Vite/Oxc) khi code giao diện React JS

*   **Khái niệm mới học được (Hiểu theo ý mình):**
    *   **JSX (JavaScript XML)** là cú pháp mở rộng của React JS, bắt buộc tất cả các thẻ mở phải có thẻ đóng tương ứng và khớp chính xác kiểu thẻ (ví dụ: thẻ mở `<a>` phải đi với thẻ đóng `</a>`, không được dùng `</Link>`).
    *   **Vite Oxc Compiler** là trình biên dịch mã nguồn của Vite cho JS/JSX. Khi build hoặc chạy dev, nếu phát hiện thẻ đóng không khớp thẻ mở (tag mismatch), nó sẽ ném ra lỗi `[PARSE_ERROR] Expected corresponding JSX closing tag` và dừng ngay tiến trình biên dịch frontend.

*   **Đoạn code / Cấu hình tâm đắc:**
    *   *Sửa lỗi tag mismatch (Thẻ mở HTML <a> đi kèm thẻ đóng </a>):*
    ```jsx
    // SAI: Thẻ mở <a> đi với thẻ đóng </Link>
...
*   **Bug / Khó khăn gặp phải & Cách khắc phục:**
    *   *Mô tả lỗi:* Trình duyệt và console Vite báo lỗi:
        `[PARSE_ERROR] Expected corresponding JSX closing tag for 'a'. Expected </a> but found </Link> in src/pages/Jobs.jsx`.
    *   *Cách giải quyết:* Lỗi do copy-paste từ template có sẵn, thẻ mở sử dụng thẻ HTML truyền thống `<a>` nhưng thẻ đóng copy nhầm thành thẻ đóng React Router `</Link>`. Đã rà soát lại toàn bộ các thẻ anchor trong file `Jobs.jsx` và chỉnh sửa các thẻ đóng `</Link>` lỗi thành `</a>`.
...
*   **Cách xử lý:** 
    *   Sử dụng dữ liệu định danh duy nhất có sẵn `list_id` từ API nguồn để tự động lắp ghép thành link URL rút gọn: `https://www.vieclamtot.com/{list_id}.htm`.
...
*   **Các hướng xử lý thực tế:**
    ...
    3.  **Dynamic Parameter (Tham số động):** Cho phép truyền `limit` và `page` trực tiếp từ Controller qua Endpoint điều khiển `GET /api/v1/crawler/crawl?source=vieclamtot&limit=50` để tăng tính tùy biến mà không cần biên dịch lại mã nguồn backend.
...
### 🛠️ NHẬT KÝ THỰC CHIẾN: KHẮC PHỤC LỖI TRÙNG LẶP DỮ LIỆU & NÂNG CẤP CƠ CHẾ PHÂN TRANG (30/06/2026)

#### 1. Sự cố trùng lặp dữ liệu và cào thiếu số lượng
*   **Sự cố:** Mặc dù đã tăng giới hạn phân trang nhưng crawler vẫn chỉ lấy được tối đa khoảng 30 tin, không bao giờ đạt mốc 50 tin mới từ TopCV.
*   **Nguyên nhân:** 
    *   *Ở phía Job Service:* Khi crawler gửi tin trùng lặp, `job-service` âm thầm trả về status `200 OK` (chỉ trả về job cũ). Crawler Service nhận được 200 OK nên vẫn tăng biến đếm `successCount++`. Khi `successCount` chạm mốc 50, crawler dừng lại nhưng thực chất phần lớn là tin cũ.
    *   *Ở phía Crawler Service:* Cào trang Landing `/viec-lam` chỉ hiển thị 6-7 tin nổi bật (Feature Jobs) trên HTML tĩnh. Khi đổi trang `?page=x`, server-side của TopCV vẫn trả về 6-7 tin đó hoặc thay đổi rất ít, còn lại là tin lặp lại, dẫn đến thiếu hụt dữ liệu mới.
*   **Giải pháp xử lý:**
    *   *Chuẩn hóa REST API:* Thay đổi logic `createJob` trong [JobServiceImpl.java](file:///K:/Project/ProjectForIntern/jobradar-backend/job-service/src/main/java/com/jobradar/job/service/JobServiceImpl.java) để kiểm tra trùng URL và ném lỗi `DuplicateResourceException` (HTTP `409 Conflict`).
    *   *Bắt lỗi cào bù tại Crawler:* Cấu hình các crawler (`TopCVCrawler`, `ViecLamTotCrawler`, `VietnamWorksCrawler`) để catch `HttpStatusCodeException` với mã 409. Khi gặp lỗi này, in log `INFO` (bỏ qua tin trùng) và **không tăng `successCount`**. Việc này bắt buộc crawler phải tiếp tục duyệt qua các trang tiếp theo để cào đủ 50 tin mới thực sự.
    *   *Phát hiện URL danh mục ẩn:* Chuyển đổi URL cào của TopCV từ Landing Page `/viec-lam` sang trang tìm kiếm chính thức **`/tim-viec-lam-moi-nhat`**. Trang này hỗ trợ SSR tĩnh render đầy đủ 28 tin mỗi trang, cung cấp nguồn dữ liệu dồi dào.

#### 2. Kỹ thuật chống lỗi ngắt quãng (Crawler Resilience)
*   **Sự cố:** Khi cào đa trang, nếu gặp lỗi mạng hoặc timeout tại một trang bất kỳ (ví dụ trang 3), khối `try-catch` lớn sẽ thực thi lệnh `break` làm dừng toàn bộ tiến trình cào, bỏ lỡ dữ liệu ở các trang sau.
*   **Giải pháp:** Thay thế `break` bằng việc tăng `page++` và `continue` trong khối catch lớn. Điều này giúp crawler bỏ qua trang lỗi tạm thời và tiếp tục cào các trang tiếp theo để thu thập đủ số lượng tin mong muốn.

#### 3. Khắc phục lỗi hụt trường (Logo, Địa điểm, Lương) bằng Smart Parser chiều sâu
*   **Sự cố:** Một số job cào về bị trống ảnh logo, trống địa điểm hoặc lương.
*   **Nguyên nhân:** 
    *   *Lỗi độ sâu DOM:* DOM của trang danh mục mới sâu hơn (6 cấp parent) so với trang Landing cũ. Vòng lặp Smart Parser cũ chỉ đi ngược tối đa 5 cấp parent nên dừng lại ở `.body` (không bao trùm thẻ `.avatar` chứa logo hay thẻ `.info` chứa địa điểm và lương).
    *   *Lỗi Lazy Loading:* Ảnh logo được lưu trong thuộc tính `data-src` để lazy-load, trong khi code cũ chỉ đọc thuộc tính `src` (chứa placeholder rỗng).
*   **Giải pháp:**
    *   Tăng giới hạn đi ngược lên parent của Smart Parser từ `5` lên **`10` cấp** trong [TopCVCrawler.java](file:///K:/Project/ProjectForIntern/jobradar-backend/crawler-service/src/main/java/com/jobradar/crawler/service/TopCVCrawler.java) để bao phủ hoàn toàn thẻ cha gốc `.job-item-search-result`.
    *   Cấu hình logo parser đọc thuộc tính `data-src` trước, nếu không có mới fallback về `src`.

#### 4. Lệch pha giữa `@Column(unique = true)` và Database thực tế (Bài học về Hibernate DDL-Auto)
*   **Sự cố:** Cột `jobUrl` trong code Java đã khai báo `@Column(unique = true)` nhưng database PostgreSQL thực tế vẫn ghi nhận bản ghi trùng lặp URL.
*   **Nguyên nhân:**
    *   *Hạn chế của Hibernate Update:* Cấu hình `spring.jpa.hibernate.ddl-auto=update` chỉ có tác dụng tạo khóa UNIQUE khi khởi tạo bảng mới. Đối với bảng đã tồn tại từ trước, Hibernate sẽ không tự động chỉnh sửa (ALTER TABLE) cột cũ để thêm UNIQUE constraint.
    *   *Xung đột đồng thời (Race Condition):* Khi crawler chạy đa luồng đồng thời, việc kiểm tra trùng lặp ở tầng ứng dụng bằng `findByJobUrl(url)` bị vô hiệu hóa vì cả hai luồng kiểm tra cùng một thời điểm khi tin chưa được lưu, dẫn đến lưu trùng thành công.
*   **Giải pháp xử lý:**
    *   Chạy SQL dọn dẹp các bản ghi trùng hiện có:
        `DELETE FROM jobs a USING jobs b WHERE a.id > b.id AND a.job_url = b.job_url;`
    *   Tạo khóa UNIQUE ở mức vật lý trực tiếp trên database:
        `ALTER TABLE jobs ADD CONSTRAINT unique_job_url UNIQUE (job_url);`
    *   **Bài học kinh nghiệm:** Luôn thiết lập UNIQUE constraint ở mức Database vật lý đối với các trường định danh duy nhất (như URL, email, username) để làm lá chắn cuối cùng chống lại Race Condition. Không nên ỷ lại hoàn toàn vào Hibernate DDL-auto update.
